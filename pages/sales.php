...
<html lang="en">
  ...
  <div class="collapse navbar-collapse" id="navbarText">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item active">
        <a class="nav-link <?php if ( strpos( $_SERVER['REQUEST_URI'], 'stock' ) ) {?> active 
				<?php } ?>" href="?page=stock" id="stock">SALES <span class="sr-only">(current)</span></a>
    </ul>
    ...
    <div class="col-lg-4"></div>
		<div class="col-lg"><h3 style="text-align: center; font-weight: bold;">ALL SALES</h3></div>
	<div class="col-lg-3">
		<?php
		...
		if (mysqli_num_rows($result) > 0) {
			while($row = mysqli_fetch_assoc($result)) {
				$price = ($row['Rapnet']*(1+$row['Discount']))*$row['Carat'];
				$revenue = $revenue + $price;
			}
		}
		?>
		<h3 style="text-align: center; ofnt-weight: bold; color: green;">REVENUE = <?php echo $revenue ?></h3>
		</div>
    
    <div class="container">
    	<form class="form-horizontal" action="sales.php" method="POST">
        	<div class="form-group">
...
//FILTERING DATA
	...
	if (mysqli_num_rows($result) > 0) {
	  ...
	  while($row = mysqli_fetch_assoc($result)) {
	   ...
	   <td>".$row["Clarity"]."</td>
	   <td>".$price. "</td>
	   <td><a href='saledelete.php?id=".$row["Stock ID"]." & 
	   cut=".$row["Cut"]." &
	   color=".$row["Color"]." &
	   carat=".$row["Carat"]." &
	   clarity=".$row["Clarity"]." &
	   ...
	   sell=".$row["Sell"]." &
	   total=".$row['Total']."
	   '>Undo Sale</a> / <a href='delete.php?id=".$row["Stock ID"]."' style='color:red;'>Delete</a></td></tr>";
	  }
	  echo "</table>";
	}
mysqli_close($conn);
